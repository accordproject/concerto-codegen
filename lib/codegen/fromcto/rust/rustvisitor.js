...
class RustVisitor {
    ...
    visitModelFile(modelFile, parameters) {
        ...
        parameters.fileWriter.writeLine(0, 'use chrono::{ DateTime, Utc };');
        ...
        // Check if any class has subclasses (needs HashMap + serde_json for flatten)
        const hasBaseTypes = modelFile
            .getAllDeclarations()
            .filter((declaration) => declaration.isClassDeclaration?.())
            .some((declaration) => {
                const subs = declaration.getDirectSubclasses?.() || [];
                return subs.length > 0;
            });

        if (hasMapDeclaration || hasBaseTypes) {
            parameters.fileWriter.writeLine(
                0,
    ...
    visitClassDeclaration(classDeclaration, parameters) {
        ...
        // Add #[serde(flatten)] catch-all field for base types with subtypes.
        // When a Concerto class has subclasses, the JSON AST for those subtypes
        // contains fields from both the parent and child. Without flatten, the
        // parent struct silently drops the child-specific fields during deserialization.
        const subclasses = classDeclaration.getDirectSubclasses?.() || [];
        if (subclasses.length > 0) {
            parameters.fileWriter.writeLine(1, '');
            parameters.fileWriter.writeLine(1, '#[serde(');
            parameters.fileWriter.writeLine(2, 'flatten,');
            parameters.fileWriter.writeLine(1, ')]');
            parameters.fileWriter.writeLine(
                1,
        ...
        // Generate union enum for polymorphic types when subclasses exist
        // Only generate unions when explicitly requested via flattenSubclassesToUnion parameter
        // and when all subclasses are also in the current model file
        if (parameters.flattenSubclassesToUnion) {
            const subclasses = classDeclaration.getDirectSubclasses?.();
            const currentModelFile = classDeclaration.getModelFile();

            // Filter subclasses to only include those in the same model file
            const localSubclasses =
                subclasses?.filter(
                    (subclass) => subclass.getModelFile() === currentModelFile
                ) || [];

            if (localSubclasses.length > 0) {
                const unionName = `${classDeclaration.getName()}Union`;

                parameters.fileWriter.writeLine(
                    0,
                    '#[derive(Debug, Clone, Serialize, Deserialize)]'
                );
                parameters.fileWriter.writeLine(0, '#[serde(tag = "$class")]');
                parameters.fileWriter.writeLine(0, `pub enum ${unionName} {`);

                // Include the base class itself if it's not abstract
                if (!classDeclaration.isAbstract?.()) {
                    const baseClassName = classDeclaration.getName();
                ...
                // Add each subclass (filter out enum subclasses as they're not relevant for unions)
                localSubclasses
                    .filter((subclass) => !subclass.isEnum?.())
                    .forEach((subclass) => {
                        const subclassName = subclass.getName();
                        const fullyQualifiedName =
                            subclass.getFullyQualifiedName();
                        parameters.fileWriter.writeLine(
                            1,
                            `#[serde(rename = "${fullyQualifiedName}")]`
                        );
    ...
    visitField(field, parameters) {
        ...
        // Check for @literal decorator
        const hasLiteral = decorators.some(
            (decorator) => decorator.getName() === 'literal'
        );

        // Handle @literal decorator - generates constant field assignment
        if (hasLiteral) {
            const literalDecorator = decorators.find(
                (decorator) => decorator.getName() === 'literal'
            );
            const decoratorArguments = literalDecorator.getArguments();

            if (decoratorArguments && decoratorArguments.length > 0) {
                const enumValue = decoratorArguments[0];
                const fieldType = field.getType();

                // For literal decorators, we generate a constant assignment
                // Example: pub literal_test: MyEnum = MyEnum::MyValue;
                parameters.fileWriter.writeLine(
                    1,
        ...
        // Determine if we should use union types
        let useUnion = false;

        // Check for @union decorator
        const hasUnion = decorators.some(
            (decorator) => decorator.getName() === 'union'
        );

        // Check if this field's type has subclasses and we should flatten to union
        if (!field.isPrimitive?.() && parameters.flattenSubclassesToUnion) {
            try {
                const fieldType = field
                    .getParent()
                    .getModelFile()
                    .getModelManager()
                    .getType(field.getFullyQualifiedTypeName());
                const subclasses = fieldType.getDirectSubclasses?.();
                if (subclasses && subclasses.length > 0) {
                    const isEnumRef = fieldType.isEnum?.();
                    const isMapRef = fieldType.isMapDeclaration?.();
                    useUnion = !(isEnumRef || isMapRef);
                }
            } catch (e) {
                // If we can't resolve the type, don't use union
                useUnion = false;
            }
        }

        // Override with explicit @union decorator
        if (hasUnion) {
            useUnion = true;
        }

        let type = this.toRustType(field.type, useUnion);
        if (field.isArray?.()) {
            type = `Vec<${type}>`;
        }
     ...
     * @private
     */
    toRustType(type, useUnion = false) {
        // Validate input
        ...
        default: {
            ...
            // Add Union suffix if requested
            const unionSuffix = useUnion ? 'Union' : '';
            return `${type}${unionSuffix}`;
        }
        }
    }
