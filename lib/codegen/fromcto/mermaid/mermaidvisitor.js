...
class MermaidVisitor extends DiagramVisitor {
    ...
    visitMapDeclaration(mapDeclaration, parameters) {
        let fqn = this.escapeString(mapDeclaration.getFullyQualifiedName());
        let keyType = mapDeclaration.getKey().getType();
        let valueType = mapDeclaration.getValue().getType();

        const typeDeclaration = mapDeclaration.getModelFile().getType(valueType);
        // if value is a complex type other than Scalar, illustrate the relationship
        if (!ModelUtil.isPrimitiveType(valueType) && !typeDeclaration.isScalarDeclaration?.()) {
            const type = `"1" ${DiagramVisitor.AGGREGATION} "0..*"`;
            valueType = mapDeclaration.getModelFile().getFullyQualifiedTypeName(valueType);
            parameters.fileWriter.writeLine(0, `${fqn} ${type} ${this.escapeString(valueType)}`);
        } else {
            parameters.fileWriter.writeLine(0, `${fqn} 0..* <${keyType}, ${valueType}>`);
        }
    }
