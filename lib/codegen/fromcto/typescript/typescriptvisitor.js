...
class TypescriptVisitor {
    ...
    visitModelFile(modelFile, parameters) {
        ...
        modelFile.imports.forEach((imp) => {
            if (imp['$class'] == 'concerto.metamodel@1.0.0.ImportTypes' && imp.aliasedTypes) {
                imp.aliasedTypes.forEach((type) => {
                    aliasedTypesMap.set(`I${type.name}`, `I${type.aliasedName}`);
                });
            }
        }); 
            ...
            .forEach(namespace => {
                ...
                if (propertyTypeNames) {
                    const csvPropertyTypeNames = Array.from(propertyTypeNames).map((type) =>
                        aliasedTypesMap.has(type) ? `${type} as ${aliasedTypesMap.get(type)}` : type
                    );
    ...
    visitClassDeclaration(classDeclaration, parameters) {
        ...
        let superType = ' ';
        if (classDeclaration.getSuperType()) {
            superType = `I${ModelUtil.getShortName(classDeclaration.getSuperType())}`;
            superType = ` extends ${parameters.aliasedTypesMap.has(superType) ? parameters.aliasedTypesMap.get(superType) : superType} `;
        }
    ...
    visitField(field, parameters) {
        ...
        const isMapRef = field.isPrimitive() ? false : field.getParent().getModelFile().getModelManager().getType(field.getFullyQualifiedTypeName?.()).isMapDeclaration?.();
