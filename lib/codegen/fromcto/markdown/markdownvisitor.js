...
class MarkdownVisitor extends BaseVisitor {
    ...
    visitClassDeclaration(classDeclaration, parameters) {
        ...
        if(classDeclaration.getSuperType() &&
            ...
            parameters.fileWriter.writeLine(0, ` - Inherits properties from <code>${classDeclaration.getSuperType()}</code>`);
        }
        parameters.fileWriter.writeLine(0, '');

        if (classDeclaration.getProperties().length > 0) {
            parameters.fileWriter.writeLine(0, '| Field | Type | Required | Identifier | Reference | Description | Contraints | ');
            parameters.fileWriter.writeLine(0, '| ------ | ------------------- | --- | --- | --- | ------------ | --------- |');
            classDeclaration.getOwnProperties().forEach((property) => {
                property.accept(this, parameters);
            });
        } else {
            parameters.fileWriter.writeLine(0, 'No properties.');
        }
        parameters.fileWriter.writeLine(0, '');

        const exampleDecorator = classDeclaration.getDecorator('example');
        if (exampleDecorator) {
            parameters.fileWriter.writeLine(0, '#### Example Usage');
            parameters.fileWriter.writeLine(0, '```cs');
            parameters.fileWriter.writeLine(0, exampleDecorator.getArguments()[0]);
            parameters.fileWriter.writeLine(0, '```');
            parameters.fileWriter.writeLine(0, '');
        }

        const systems = classDeclaration.getDecorator('usedBy');
        if (systems) {
            parameters.fileWriter.writeLine(0, '#### Used By');
            parameters.fileWriter.writeLine(0, '| |');
            parameters.fileWriter.writeLine(0, '| - |');
            for (const system of exampleDecorator.getArguments()) {
    ...
    visitField(field, parameters) {
        ...
        if(validator) {
            if(validator.getMinLength?.()) {
                contraints += `Minimum Length: ${validator.getMinLength()} \n`;
            }
            if(validator.getMaxLength?.()) {
                contraints += `Maximum Length: ${validator.getMaxLength()} \n`;
            }
            if (validator.getRegex?.()) {
                contraints += `Matches Regular Expression: \`${validator.getRegex()}\` \n`;
            }
            if(validator.getLowerBound?.()) {
                contraints += `Minimum Value: ${validator.getLowerBound()} \n`;
            }
            if(validator.getUpperBound?.()) {
                contraints += `Maximum Value: ${validator.getUpperBound()} \n`;
            }
        }

        parameters.fileWriter.writeLine(0, `| ${field.getName()} | <code>${type}</code> | ${!field.isOptional() ? '✔️' : ''} | ${isIdentifier ? '✔️' : ''} | ${field.isRelationship?.() ? '✔️' : ''}  |${descriptionDecorator?.getArguments()[0] || ''} | ${contraints} |`);
