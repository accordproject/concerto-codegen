...
class MarkdownVisitor extends BaseVisitor {
    ...
    visitField(field, parameters) {
        ...
        parameters.fileWriter.writeLine(0, `| ${field.getName()} | <code>${type}</code> | ${!field.isOptional() ? '✔️' : ''} | ${isIdentifier ? '✔️' : ''} | ${field.isRelationship?.() ? '✔️' : ''}  |${descriptionDecorator?.getArguments()[0] || ''} | ${contraints.join('<br>')} |`);
    }
