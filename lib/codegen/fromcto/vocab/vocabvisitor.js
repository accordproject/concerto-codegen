...
/**
 * Convert the contents of a ModelManager to Vocab YAML.
...
class VocabVisitor {
    ...
    visitDeclaration(declaration, parameters) {
        ...
        const declarationVocab = VocabularyManager.englishMissingTermGenerator(null, null, declarationName);

        parameters.fileWriter.writeLine(0, `  - ${declarationName}: ${declarationVocab}`);
    ...
    visitField(field, parameters) {
        const classDeclarationName = field.getParent().getName();
        const fieldName = field.getName();
        const fieldVocab = VocabularyManager.englishMissingTermGenerator(null, null, classDeclarationName, fieldName);
        parameters.fileWriter.writeLine(0, `      - ${fieldName}: ${fieldVocab}`);

        return null;
    }
}

module.exports = VocabVisitor;
