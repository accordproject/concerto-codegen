...
class GoLangVisitor {
    ...
    visitField(field, parameters) {
        ...
        if(ModelUtil.isMap(field)) {
            ...
            let mapKeyType   = mapDeclaration.getKey().getType();
            let mapValueType = mapDeclaration.getValue().getType();

            if (mapDeclaration.getModelFile().getType(mapKeyType).isScalarDeclaration?.()) {
                mapKeyType = mapDeclaration.getModelFile().getType(mapKeyType).getType();
            }

            const goKeyType = this.toGoType(mapKeyType);

            if (mapDeclaration.getModelFile().getType(mapValueType).isScalarDeclaration?.()) {
                mapValueType = mapDeclaration.getModelFile().getType(mapValueType).getType();
            }

            const goValueType = this.toGoType(mapValueType);

            parameters.fileWriter.writeLine(1, `${field.getName()} := make(map[${goKeyType}]${goValueType})` + ' `json:"' + field.getName() + '"`');
            return null;
        }
    ...
    containsDateTimeField(modelFile) {
        let classDeclarations = modelFile.getAllDeclarations()
            .filter(declaration => !declaration.isScalarDeclaration?.() && !declaration.isMapDeclaration?.());
