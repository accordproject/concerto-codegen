...
class AvroVisitor {
    ...
    visitField(field, parameters) {
        ...
        if (ModelUtil.isMap(field)) {
            ...
            if (ModelUtil.isScalar(mapDeclaration.getValue())) {
                const scalarDeclaration = mapDeclaration.getModelFile().getType(mapValueType);
                mapValueType = scalarDeclaration.getType();
            }

            const avroValueType = this.toAvroType(mapValueType);

            parameters.fileWriter.writeLine(2, `map<${avroValueType}> ${field.getName()};`); // key is always inferred to be string.
