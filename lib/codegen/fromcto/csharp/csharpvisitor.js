...
class CSharpVisitor {
    ...
    resolveMapSide(side, modelFile, parameters) {
        // concerto-core bug: MapValueType.processType() has no case for
        // RelationshipMapValueType, so this.type is never assigned and getType()
        // returns undefined. RelationshipMapValueType shares the same AST shape as
        // ObjectMapValueType (ast.type.name holds the target type name), so we fall
        // back to the AST directly until the upstream fix lands.
        // TODO: remove the AST fallback and rely solely on getType() once the concerto-core bug is fixed.
        const typeName = side.getType() ?? side.ast?.type?.name;
