...
class PlantUMLVisitor extends DiagramVisitor {
    ...
    visitMapDeclaration(mapDeclaration, parameters) {
        parameters.fileWriter.writeLine(0, 'class ' + this.escapeString(mapDeclaration.getFullyQualifiedName()) +  ' << (M,pink) >> '  + '{' );
        parameters.fileWriter.writeLine(1, `+ ${mapDeclaration.getKey().getType()}` );
        parameters.fileWriter.writeLine(1, `+ ${mapDeclaration.getValue().getType()}` );
