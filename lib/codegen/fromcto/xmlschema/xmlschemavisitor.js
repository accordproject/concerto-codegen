    ...
    toXsType(type, mapElement, mapDeclaration) {
        ...
        default:
            if(!ModelUtil.getNamespace(type)) {
                const typeDeclaration = mapDeclaration.getModelFile().getAllDeclarations().find(d => d.name === type);
                if (typeDeclaration?.isClassDeclaration?.() || typeDeclaration?.isScalarDeclaration?.()) {
