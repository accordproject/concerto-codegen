...
class JavaVisitor {
    ...
    startClassFile(clazz, parameters) {
        const { name, version } = ModelUtil.parseNamespace(clazz.getNamespace());
        const namespace = version ? `${name}@${version}` : name;
    ...
    visitEnumDeclaration(enumDeclaration, parameters) {
        ...
        if (properties.length === 0) {
            ...
            properties.forEach((property, index) => {
                ...
                parameters.fileWriter.writeLine(1, property.getName() + suffix);
            });
        }
    ...
    visitClassDeclaration(classDeclaration, parameters) {
        ...
        const writeImport = (namespace, typeName) => {
            const { name: javaBaseNamespace } = ModelUtil.parseNamespace(namespace);
            const { name: classBaseNamespace } = ModelUtil.parseNamespace(classDeclaration.getNamespace());
            const imported = `${namespace}.${typeName}`;
            if (!importedTypes.has(imported) && javaBaseNamespace !== classBaseNamespace) {
        ...
        classDeclaration.getModelFile().getImports().forEach((imported) => {
            if (!this.shouldImportType(classDeclaration.getModelFile(), imported)) {
                return;
            }
            const { name, version } = ModelUtil.parseNamespace(ModelUtil.getNamespace(imported));
            const namespace = version ? `${name}@${version}` : name;
            const typeName = ModelUtil.getShortName(imported);
            writeImport(namespace, typeName);
        });

        classDeclaration.getOwnProperties().forEach((property) => {
            if (ModelUtil.isMap(property) && property.getModelFile?.()) {
                this.getJavaMapMemberImports(property).forEach(({ namespace, typeName }) => {
                    writeImport(namespace, typeName);
                });
            }
        });
...
`
    ...
    visitField(field, parameters) {
        ...
        const isMap = ModelUtil.isMap(field);
        if (isMap) {
            fieldType = this.toJavaMapType(field);
        } else {
        ...
        if (parameters.mode) {
            ...
            default:
                if (isMap) {
                    parameters.fileWriter.writeLine(1, `private ${fieldType} ${field.getName()} = new HashMap<>();`);
                } else {
     ...
     * @private
     */
    shouldImportType(modelFile, imported) {
        const modelManager = modelFile.getModelManager?.();
        const declaration = modelManager?.getType?.(imported);
        return !(declaration?.isScalarDeclaration?.() || declaration?.isMapDeclaration?.());
    }
    ...
    toJavaMapType(field) {
        ...
        const keyType = this.toJavaMapMemberType(mapDeclaration, mapDeclaration.getKey());
        const valueType = this.toJavaMapMemberType(mapDeclaration, mapDeclaration.getValue());
     ...
     * @private
     */
    toJavaMapMemberType(mapDeclaration, member) {
        let type = member.getType();
        if (!ModelUtil.isPrimitiveType(type)) {
            const declaration = mapDeclaration.getModelFile().getType(type);
            if (declaration?.isScalarDeclaration?.()) {
     ...
     * @private
     */
    getJavaMapMemberImports(field) {
        const mapDeclaration = field.getModelFile().getType(field.getType());
        return [mapDeclaration.getKey(), mapDeclaration.getValue()]
            .map((member) => {
                ...
                const declaration = mapDeclaration.getModelFile().getType(type);
                if (declaration?.isScalarDeclaration?.() || declaration?.isMapDeclaration?.()) {
                    return null;
                }

                return {
                    namespace: mapDeclaration.getModelFile().getNamespace(),
                    typeName: type
                };
            })
            .filter(Boolean);
