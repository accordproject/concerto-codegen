...
class JavaVisitor {
    ...
    visitClassDeclaration(classDeclaration, parameters) {
        ...
        const writeImport = (namespace, typeName) => {
            const { name: javaNamespace } = ModelUtil.parseNamespace(namespace);
            const { name: classNamespace } = ModelUtil.parseNamespace(classDeclaration.getNamespace());
            const imported = `${javaNamespace}.${typeName}`;
            if (!importedTypes.has(imported) && javaNamespace !== classNamespace) {
