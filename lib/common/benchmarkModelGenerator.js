...
class BenchmarkModelGenerator {
    ...
    generateConcertoModels({
        ...
        if (generateUpToSize > v8.getHeapStatistics().total_available_size) {
            throw new Error('The requested model size exceeds the available heap size and cannot be created.');
        }
